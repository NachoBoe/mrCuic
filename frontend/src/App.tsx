...
interface Activity {
  type: string;
  content: any; // Change to any or specific types based on your use case
  sender: string;
}
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (event) => {
      ...
      if (activity.type === 'suggestion') {
        setSuggestions(activity.content); // Assuming content is of type string[]
      } else {
  ...
  useEffect(() => {
    messages.forEach((msg, index) => {
      if (msg.type === 'adaptive_card' && msg.content) {
        const cardElements = parseAdaptiveCard(msg.content);
  ...
  const handleInputChange = (cardId: string, id: string, value: any, required: boolean) => {
    ...
    const cardElements = parseAdaptiveCard(messages[parseInt(cardId)].content!);
    ...
    return elements.every((element) => {
      if (element instanceof FillIn || element instanceof Choice) {
        if (element.required) {
          ...
          console.log(value)
          console.log(value && value !== '' && value !== 'Seleccionar una opción')
          return value && value !== '' && value !== 'Seleccionar una opción' && value !== 'None';
      ...
      <ChatContainer>
        ...
        <MessagesContainer>
          {messages.map((msg, index) => (
            <div key={index}>
              <SenderLabel>{msg.sender === 'user' ? 'Tú' : 'MrQuick'}</SenderLabel>
              <Message sender={msg.sender}>
                {msg.type === 'message' && typeof msg.content === 'string'
                  ? msg.content.split('\n').map((line, i) => (
                  ...
                  : null}
                {msg.type === 'adaptive_card' && msg.content && renderAdaptiveCard(msg.content, `${index}`)}
              </Message>
            </div>
          ))}
...
const parseAdaptiveCard = (cardContent: string): any[] => {
  ...
  return parsed.map((item: any) => {
    if (item.FillIn) {
      ...
      console.log('Unknown element:', item);
    }
    return null;
  });
};
...
const Message = styled.li<{ sender: string }>`
  background-color: ${({ sender }) => (sender === 'user' ? '#4a00e0' : '#333')}; /* More intense blue-violet for user */
  color: ${({ sender }) => (sender === 'user' ? 'white' : '#ddd')};
  padding: 12px;
  margin-bottom: 12px;
  border-radius: ${({ sender }) =>
    sender === 'user' ? '20px 0px 20px 20px' : '0px 20px 20px 20px'}; /* Squared corner for message direction */
  max-width: 80%;
  min-width: 30%;
  width: fit-content;
  word-break: break-word;
  text-align: ${({ sender }) => (sender === 'user' ? 'right' : 'left')};
  margin-left: ${({ sender }) => (sender === 'user' ? 'auto' : '16px')};
  margin-right: ${({ sender }) => (sender === 'user' ? '16px' : 'auto')};
